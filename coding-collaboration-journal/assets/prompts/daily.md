...
1. Read `.journal/config.toml`, `AGENTS.md`, active memory, and `docs/method/daily-runtime.md` plus `docs/method/report-contract.md`.
...
8. Retrieve only relevant historical reports and memory for multi-day continuity. Update project index, open loops, pattern evidence/counter-evidence, and experiment observations. Do not dump the report into memory.
9. Do not run broad web research during the daily job. Add a radar candidate when repeated friction, a new task class, a tool change, or an active experiment justifies it.
10. Write `run.json`, run `python3 scripts/validate_journal.py --root .`, and resolve deterministic validation failures where safe.
11. Commit only journal-owned changes using `journal: YYYY-MM-DD daily reflection`. Push according to policy. If push fails, preserve the local commit and update pending-push state.
12. After the daily artifact is safe, check review boundaries. If the configured weekly or monthly review is due and missing, execute the corresponding automation prompt in the same unattended run, with its own validation and commit. This lets one durable daily scheduler keep the full system moving.
