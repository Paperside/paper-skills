...
| Install approval gate | Pass in local tests | Non-dry-run bootstrap refuses to write without explicit `--yes`; onboarding requires a status checklist and user-confirmed install plan. | Agent compliance still depends on reading the Skill, but the script now blocks accidental writes. |
...
| Lightweight language templates | Pass in local tests | `zh*` and `en*` select localized README, AGENTS, CLAUDE, report template, and automation prompts; unsupported tags preserve config and fall back to English templates. | Method docs remain the canonical English runtime contract. |
