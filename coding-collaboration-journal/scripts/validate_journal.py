...
DEFAULT_SESSION_BRIEFING_PLACEHOLDER = (
...
def meaningful_session_briefing(text: str) -> bool:
    stripped = text.strip()
    if not stripped:
        return False
    content = stripped.replace("# Session Briefing", "").strip()
    return bool(content and content != DEFAULT_SESSION_BRIEFING_PLACEHOLDER)


def validate_memory_runtime(root: Path, config: dict[str, Any], items: list[dict[str, Any]]) -> None:
    ...
    try:
        ...
        hard_limit = 10_000
    hard_limit = max(1000, hard_limit)

    briefing_path = root / "memory" / "session-briefing.md"
    if not briefing_path.is_file():
        return
    try:
        briefing = briefing_path.read_text(encoding="utf-8", errors="replace")
    except OSError as exc:
        add(items, "error", "memory-briefing-unreadable", str(exc), briefing_path)
        return
    if len(briefing) > hard_limit:
        add(
            items,
            "error",
            "memory-briefing-over-budget",
            f"memory/session-briefing.md is {len(briefing)} chars; hard limit is {hard_limit}",
            briefing_path,
        )
    if bool(memory_config.get("inject_on_session_start", False)) and not meaningful_session_briefing(briefing):
        add(
            items,
            "warning",
            "memory-briefing-placeholder",
            "Session injection is enabled, but memory/session-briefing.md has no maintained briefing yet",
            briefing_path,
        )
...
def validate(root: Path) -> dict[str, Any]:
    ...
    if config_path.is_file():
        try:
            ...
            validate_memory_runtime(root, config, items)
